...
**AlfaCRM НЕ ВІДАЄ історію змін статусів** → треба ВІДНОВИТИ історію:
- AlfaCRM API повертає тільки поточний статус ліда
- Треба зберігати історію змін локально (в нашій БД)
- При кожному запиті:
  1. Отримуємо поточні статуси з AlfaCRM
  2. Порівнюємо з попередніми значеннями в нашій БД
  3. Фіксуємо зміни статусів
  4. Враховуємо історію при обчисленні метрик
