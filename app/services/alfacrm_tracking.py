...
ALFACRM_STATUS_TO_GROUP = {
    # Недозвон (об'єднання всіх варіантів)
    11: "Недозвон",
    10: "Недозвон",
    27: "Недозвон",
    18: "Недозвон",
    ...
    # Встановлено контакт
    1: "Встановлено контакт",
    32: "Встановлено контакт",
    43: "Встановлено контакт",
    22: "Встановлено контакт",

    # Зник після контакту
    26: "Зник після контакту",
    44: "Зник після контакту",

    # В ОПРАЦЮВАННІ (агрегована група)
    12: "В опрацюванні",  # Розмовляли, чекаємо відповідь
    6: "В опрацюванні",   # Чекає пробного
    24: "В опрацюванні",  # Розмовляли чекаємо відповіді (втор.)
    34: "В опрацюванні",  # Чекає пробного (втор.)
    5: "В опрацюванні",   # Не відвідав пробне
    36: "В опрацюванні",  # Не відвідав пробне (втор.)
    8: "В опрацюванні",   # Опрацювати заперечення
    49: "В опрацюванні",  # Опрацювати заперечення (втор.)
...
AGGREGATED_STATUSES = [
    ...
    "Зник після контакту",
    "В опрацюванні",
    "Призначено пробне",      # Cumulative: всі хто >= цього етапу
    "Проведено пробне",        # Cumulative: всі хто >= цього етапу
    "Чекає оплату",            # Cumulative: всі хто >= цього етапу
    "Отримана оплата",         # Тільки фактичні продажі
...
def track_campaign_leads(
    ...
    status_counts["Призначено пробне"] = 0
    status_counts["Проведено пробне"] = 0
    status_counts["Чекає оплату"] = 0
    status_counts["Отримана оплата"] = 0

    # Ліди в статусі "Призначено пробне"
    prizn_count = len(trial_funnel_levels["Призначено пробне"])
    status_counts["Призначено пробне"] += prizn_count

    # Ліди в статусі "Проведено пробне"
    prov_count = len(trial_funnel_levels["Проведено пробне"])
    status_counts["Проведено пробне"] += prov_count
    status_counts["Призначено пробне"] += prov_count  # +1 на попередній етап

    # Ліди в статусі "Чекає оплату"
    chek_count = len(trial_funnel_levels["Чекає оплату"])
    status_counts["Чекає оплату"] += chek_count
    status_counts["Проведено пробне"] += chek_count  # +1 на попередній етап
    status_counts["Призначено пробне"] += chek_count  # +1 на попередній етап

    # Ліди в статусі "Отримана оплата"
    opl_count = len(trial_funnel_levels["Отримана оплата"])
    status_counts["Отримана оплата"] += opl_count
    # ПРОПУСКАЄМО "Чекає оплату"! (як вказав користувач)
    status_counts["Проведено пробне"] += opl_count  # +1 на попередній етап
    status_counts["Призначено пробне"] += opl_count  # +1 на попередній етап
