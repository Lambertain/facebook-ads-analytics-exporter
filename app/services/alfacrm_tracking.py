...
ALFACRM_STATUS_MAPPING = {
    ...
    2: "Встанов. контакт (ЦА)",            # Установлен контакт (целевая аудитория)
    3: "В опрацюванні (ЦА)",               # В обработке (целевая аудитория)
    4: "Назначений пробний",                # Назначен пробный урок
    5: "Проведений пробний",                # Проведен пробный урок
    6: "Купили (ЦА)",                       # Продажа (целевая аудитория)
    7: "Архів",                             # Архив/Отказ
    8: "Недзвін",                           # Недозвон
...
def calculate_conversion_rate(funnel_stats: Dict[str, int]) -> float:
    ...
    converted = funnel_stats.get("Купили (ЦА)", 0)
...
def calculate_target_leads_percentage(funnel_stats: Dict[str, int]) -> float:
    ...
    # Целевые = все кроме "Не розібрані" и "Недзвін"
    not_processed = funnel_stats.get("Не розібрані", 0)
...
def calculate_trial_conversion(funnel_stats: Dict[str, int]) -> float:
    ...
    Returns:
        Процент конверсии (0-100)
    """
    trial_completed = funnel_stats.get("Проведений пробний", 0)
    converted = funnel_stats.get("Купили (ЦА)", 0)
